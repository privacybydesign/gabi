...
type (
	ValidKeyProofStructure struct {
		...
		pPprimeRel zkproof.RepresentationProofStructure
		qQprimeRel zkproof.RepresentationProofStructure
		pQNRel     zkproof.RepresentationProofStructure
...
func (s *ValidKeyProofStructure) BuildProof(Pprime *big.Int, Qprime *big.Int) ValidKeyProof {
	...
	g, gok := zkproof.BuildGroup(GroupPrime)
	if !gok {
	...
	bases := zkproof.NewBaseMerge(&g, &PSecret, &QSecret, &PprimeSecret, &QprimeSecret)
	secrets := zkproof.NewSecretMerge(&PSecret, &QSecret, &PprimeSecret, &QprimeSecret, &PQNRel)
...
func (s *ValidKeyProofStructure) VerifyProof(proof ValidKeyProof) bool {
	...
	// Rebuild group
	g, gok := zkproof.BuildGroup(proof.GroupPrime)
	if !gok {
		return false
	}
	...
	bases := zkproof.NewBaseMerge(&g, &proof.PProof, &proof.QProof, &proof.PprimeProof, &proof.QprimeProof)
	proofs := zkproof.NewProofMerge(&proof.PProof, &proof.QProof, &proof.PprimeProof, &proof.QprimeProof, &proof.PQNRel)
